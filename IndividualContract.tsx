...
const IndividualContract: React.FC<IndividualContractProps> = ({ data }) => {
  ...
  const fixedItems = data.items.filter(i => i.category === '請負型業務');
  const quasiItems = data.items.filter(i => i.category === '準委任型業務');
  ...
  const pageClass = "a4-container shadow-lg print:shadow-none mb-10 text-slate-900 relative min-h-[297mm] flex flex-col pt-[20mm] pb-[20mm]";
  const articleTitleClass = "font-bold border-b border-slate-900 mb-2 font-sans text-[12pt] pb-0.5 mt-6 uppercase tracking-wider";
  const tableThClass = "bg-slate-100 border border-slate-900 p-2 text-[10pt] font-bold text-center font-sans";
  const tableTdClass = "border border-slate-900 p-2 text-[10pt] leading-relaxed";
  ...
  return (
    <div className="serif-font text-[11pt]">
      ...
      <div className={pageClass}>
        ...
        <div className="border border-slate-900 p-4 mb-8 bg-white space-y-3">
          <div>
            <strong>案件名：</strong>
        ...
        <p className="mb-3">本契約における委託業務の性質は、以下の通りとする。</p>
        <div className="flex gap-8 justify-center py-4 border border-slate-200 rounded mb-6">
        ...
        <p className="mb-2">受託者は、委託者の指示に基づき、以下の業務を遂行する。</p>
        <div className="border border-slate-900 p-4 min-h-[120px] space-y-2 mb-2">
          ...
          <p className="text-[9pt] text-slate-500 mt-4">※詳細な仕様は、別途合意した見積書（番号:{data.estimateNumber}）または仕様書による。</p>
        </div>

        <h2 className={articleTitleClass}>第3条 （成果物の範囲）</h2>
        <p className="mb-2">本業務における納品対象となる成果物の範囲は以下の通りとする。</p>
            ...
            <tr>
              ...
              <td className={tableTdClass}>
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-[8pt] text-slate-400 mt-1">※納品する場合、別途費用が発生する場合がある。</p>
                </div>
              </td>
            </tr>
          </tbody>
        ...
        <p className="mb-2 leading-relaxed">委託料に含まれる修正対応の範囲は以下の通りとする。これを超える修正、または仕様確定後の大幅な変更については、第5条または第7条に基づき追加費用を請求する場合がある。</p>
      ...
      <div className={pageClass}>
        ...
        <p className="mb-3">準委任型業務（ディレクション、PM等）の委託料および稼働条件は、以下のいずれかのパターンを適用する。</p>
        ...
        <div className="p-4 border border-slate-300 bg-slate-50 text-[9pt] mb-8 leading-relaxed text-slate-900">
          <p className="font-bold mb-2">【役割別単価表（参考値）】</p>
      ...
      <div className={pageClass}>
        ...
        <p className="mb-3">本契約の委託料総額および内訳は以下の通りとする。</p>
        ...
        <table className="w-full border-collapse border border-slate-900 mb-8 text-slate-900">
          ...
          <tbody>
            <tr className="bg-slate-50"><td className={`${tableTdClass} font-bold`} colSpan={3}>(1) 請負型業務 <span className="font-normal text-[9pt]">（成果物完成責任あり・検収対象）</span></td></tr>
            {fixedItems.map(item => (
              <tr key={item.id}>
            ...
            <tr className="bg-slate-50"><td className={`${tableTdClass} font-bold`} colSpan={3}>(2) 準委任型業務 <span className="font-normal text-[9pt]">（善管注意義務・期間対応）</span></td></tr>
            {quasiItems.map(item => (
              <tr key={item.id}>
                <td className={tableTdClass}>{item.name}</td>
            ...
            <tr className="bg-slate-100 font-bold border-t-2 border-slate-900">
              <td className="p-3 text-center border border-slate-900">合計委託料</td>
              <td className="p-3 text-center border border-slate-900 text-[9pt] font-normal italic">（消費税別途）</td>
              <td className="p-3 text-right border border-slate-900 text-lg font-sans">¥ { totalTaxExclusive.toLocaleString() }</td>
            </tr>
          </tbody>
        </table>
      ...
      <div className={pageClass}>
        ...
        <p className="mb-4 font-bold text-slate-700">本業務に関する知的財産権の取扱いは、以下のいずれかを適用する。</p>
        
        <div className={`border p-5 mb-4 rounded ${data.ipPattern === 'A' ? 'border-slate-900 bg-slate-50' : 'border-slate-200 opacity-60'}`}>
          <div className="mb-2 font-bold flex items-center gap-2">
            <CheckBox checked={data.ipPattern === 'A'} label="パターンA：著作権移転型" />
          </div>
          <p className="text-[9pt] leading-relaxed text-slate-600 pl-6">
            成果物の著作権（著作権法第27条及び第28条の権利を含む）は、全て委託者に帰属するものとし、権利の発生と同時に委託者に移転する。ただし、受託者が従前より有していた汎用的な知的財産権（プログラム、モジュール、デザインテンプレート、ノウハウ等）は受託者に留保される。
          </p>
        </div>

        <div className={`border p-5 mb-8 rounded ${data.ipPattern === 'B' ? 'border-slate-900 bg-slate-50' : 'border-slate-200 opacity-60'}`}>
          <div className="mb-2 font-bold flex items-center gap-2">
            <CheckBox checked={data.ipPattern === 'B'} label="パターンB：受託者保有 + 利用権許諾型" />
          </div>
          <p className="text-[9pt] leading-relaxed text-slate-600 pl-6">
            本業務を通じて受託者が新たに創作した部分の知的財産権は受託者に留保される。ただし、委託者は成果物を期限・地域・目的の制限なく、自ら知的財産権を保有するのと同等に利用（複製、翻案、公衆送信等）する権利を非独占的に許諾される。
          </p>
        </div>

        <h2 className={articleTitleClass}>第12条 （特記事項）</h2>
        <div className="border border-slate-900 p-5 min-h-[100px] text-slate-800 leading-relaxed text-slate-900">
          <p>1. 中間成果物（デザイン元データ等）の納品は、第3条の定めに従う。</p>
          <p>2. 本契約に定めのない事項については、基本契約の定めに従うものとする。</p>
          {/* 特記事項の表示。チェックが入っていない場合、項目3を表示しない */}
          {data.hasNotes && data.notes && (
            <p className="mt-2">3. {data.notes}</p>
          )}
        </div>
      ...
      {/* 5ページ目：署名欄 */}
      <div className={pageClass}>
        <p className="mt-20 leading-relaxed text-center mb-16 text-base text-slate-900">
          本契約の成立を証するため、本書2通を作成し、委託者及び受託者双方が記名押印（または電子署名）の上、各1通を保有する。
        </p>

        <div className="space-y-16 text-slate-900">
          <div className="text-center mb-8">
            <p className="text-xl font-bold border-b-2 border-slate-900 inline-block px-16 pb-1">
          ...
          <div className="flex flex-col gap-16 max-w-[500px] mx-auto">
            ...
            <div className="space-y-6">
              <p className="font-black text-[13pt] border-l-8 border-slate-900 pl-4">（委託者）</p>
              <div className="pl-4 space-y-4">
                <div className="flex flex-col gap-1">
                  <span className="text-[9pt] font-bold text-slate-400 uppercase tracking-tighter">住所</span>
                  <span className="border-b border-slate-400 min-h-[1.5em] block font-bold">{data.client.address || '　'}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[9pt] font-bold text-slate-400 uppercase tracking-tighter">社名</span>
                  <span className="border-b border-slate-400 min-h-[1.5em] block font-bold">{data.client.companyName || '　'}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[9pt] font-bold text-slate-400 uppercase tracking-tighter">代表者職名・氏名</span>
                  <div className="flex items-end justify-between border-b border-slate-400">
                    <span className="min-h-[1.5em] block font-bold">{data.client.representative || '　'}</span>
            ...
            {/* 受託者（下） */}
            <div className="space-y-6">
              <p className="font-black text-[13pt] border-l-8 border-slate-900 pl-4">（受託者）</p>
              <div className="pl-4 space-y-4">
                <div className="flex flex-col gap-1">
                  <span className="text-[9pt] font-bold text-slate-400 uppercase tracking-tighter">住所</span>
                  <span className="border-b border-slate-400 font-bold">〒{data.provider.zipCode} {data.provider.address}</span>
                  {data.provider.building && <span className="border-b border-slate-400 font-bold">{data.provider.building}</span>}
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[9pt] font-bold text-slate-400 uppercase tracking-tighter">社名</span>
                  <span className="border-b border-slate-400 font-bold">{data.provider.companyName}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[9pt] font-bold text-slate-400 uppercase tracking-tighter">代表者職名・氏名</span>
                  <div className="flex items-end justify-between border-b border-slate-400">
                    <span className="font-bold">代表取締役　{data.provider.representative}</span>
                    <span className="text-xs text-slate-300 font-sans border border-slate-200 rounded px-2 mb-1">印</span>
                  </div>
                </div>
              </div>

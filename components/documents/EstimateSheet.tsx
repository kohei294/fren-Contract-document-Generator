...
const EstimateSheet: React.FC<EstimateSheetProps> = ({ data, highlightSection }) => {
  const subtotal = data.items.reduce((sum, item) => sum + (item.unitPrice * item.quantity), 0);
  const totalTaxExclusive = subtotal - data.discount;
  const taxRate = data.taxRate ?? 10;
  const tax = Math.floor(totalTaxExclusive * (taxRate / 100));
    ...
    <div className="flex flex-col items-center">
      <div className={pageClass}>
        ...
        <div className="flex justify-between items-start mb-10">
          <div className={`w-[60%] section-highlight ${highlightSection === 'header' ? 'active' : ''}`}>
          ...
          <div className={`w-[35%] text-right text-xs relative space-y-1 section-highlight ${highlightSection === 'signature' ? 'active' : ''}`}>
        ...
        <div className={`border-2 border-slate-900 p-6 mb-10 text-center bg-slate-50 flex items-center justify-center gap-12 section-highlight ${highlightSection === 'discount' ? 'active' : ''}`}>
        ...
        <table className={`w-full border-collapse mb-8 border border-slate-900 section-highlight ${highlightSection === 'items' ? 'active' : ''}`}>
          <thead><tr><th className={tableThClass} style={{ width: '40%' }}>大項目</th><th className={tableThClass} style={{ width: '30%' }}>内訳</th><th className={tableThClass} style={{ width: '10%' }}>数量</th><th className={tableThClass} style={{ width: '20%' }}>金額（税別）</th></tr></thead>
          <tbody>
            {(Object.entries(groupedItems) as [string, EstimateItem[]][]).map(([cat, items]) => {
              ...
              return (<tr key={cat}><td className={tableTdClass}><strong>{cat}</strong></td><td className={tableTdClass + " text-slate-500 font-sans"}>{items.length} 項目</td><td className={tableTdClass + " text-center font-sans"}>1式</td><td className={tableTdClass + " text-right font-sans"}>¥ {catTotal.toLocaleString()}</td></tr>);
            })}
            <tr><td colSpan={3} className={tableTdClass + " text-right bg-slate-50 font-bold"}>小計（税別）</td><td className={tableTdClass + " text-right font-bold font-sans"}>¥ {subtotal.toLocaleString()}</td></tr>
            {data.discount > 0 && (<tr><td colSpan={3} className={tableTdClass + " text-right bg-slate-50 text-red-600 font-bold"}>出精お値引き</td><td className={tableTdClass + " text-right text-red-600 font-bold font-sans"}>- ¥ {data.discount.toLocaleString()}</td></tr>)}
            {/* 消費税の行：小計（値引き後）の下に配置 */}
            <tr><td colSpan={3} className={tableTdClass + " text-right bg-slate-50 font-bold"}>消費税（{taxRate}%）</td><td className={tableTdClass + " text-right font-bold font-sans"}>¥ {tax.toLocaleString()}</td></tr>
            <tr className="bg-slate-100 font-bold border-t-2 border-slate-900"><td colSpan={3} className={tableTdClass + " text-right text-[11pt]"}>合計（税込）</td><td className={tableTdClass + " text-right text-[11pt] font-sans"}>¥ {totalTaxInclusive.toLocaleString()}</td></tr>
          </tbody>
        </table>

        <div className={`border border-slate-900 p-4 text-[9pt] space-y-2 text-slate-700 font-sans mt-auto section-highlight ${highlightSection === 'notes' ? 'active' : ''}`}>
          <p className="font-bold border-b border-slate-900 pb-1 text-slate-900 text-[10pt] mb-2">【特記事項】</p>
          {dynamicNotes.map((note, index) => (<p key={index}>{index + 1}. <strong>{note.title}：</strong>{note.content}</p>))}
        </div>
      ...
      <div className={pageClass}>
        ...
        {(Object.entries(groupedItems) as [string, EstimateItem[]][]).map(([cat, items], catIdx) => (
          <div key={cat} className={`mb-10 section-highlight ${highlightSection === 'items' ? 'active' : ''}`}>
            <h3 className="text-[11pt] font-bold mb-3 border-b-2 border-slate-400 pb-1">{catIdx + 1}. {cat}</h3>
            <table className="w-full border-collapse border border-slate-900">
              <thead className="bg-slate-50 font-sans"><tr><th className={tableThClass} style={{ width: '25%' }}>項目</th><th className={tableThClass}>仕様詳細</th><th className={tableThClass} style={{ width: '15%' }}>単価</th><th className={tableThClass} style={{ width: '10%' }}>数量</th><th className={tableThClass} style={{ width: '15%' }}>金額</th></tr></thead>
              <tbody>
                {items.map((item) => (
                  <tr key={item.id}>
                    <td className={tableTdClass}><strong>{item.name}</strong>{item.subCategory && <div className="text-[7pt] text-slate-400 font-sans mt-0.5">{item.subCategory}</div>}</td>
                ...
                <tr className="bg-slate-50 font-bold"><td colSpan={4} className={tableTdClass + " text-right"}>{cat} 小計</td><td className={tableTdClass + " text-right font-sans"}>¥ {items.reduce((s, i) => s + (i.unitPrice * i.quantity), 0).toLocaleString()}</td></tr>
              </tbody>
            </table>
          </div>
        ))}
